...
use contexts::*;
use errors::FortuneError;
use helpers::{
    charge_protocol_fee, checked_add_u32, checked_add_u8, current_day_id, derive_config_address,
    derive_treasury_vault_address, ensure_system_vault, generate_fortune_index, has_pending_admin,
    require_treasury_withdrawal_safe, require_valid_system_account, withdraw_from_treasury,
};
use state::{FortuneResult, WithdrawTreasuryArgs};

declare_id!("FrtnBtnPK86hRM2pMF7FesE38MYDi59z9dMuNyfxiq");

#[program]
pub mod fortune_button {
    use super::*;
    ...
    pub fn fortune(ctx: Context<Fortune>) -> Result<()> {
        ...
        require!(
            ...
            FortuneError::DailyLimitReached
        );
        ...
        let fortune_index = generate_fortune_index(
            clock.slot,
